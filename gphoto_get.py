...
def fetch_page_content(url, verbose=False):
    ...
    try:
        ...
        log(f"Successfully fetched page content ({len(response.text)} bytes)", verbose, style="success")
...
def check_file_exists(output_dir, base_name):
    ...
    for existing_file in os.listdir(output_dir):
        ...
        if existing_file.startswith(base_name) and os.path.isfile(os.path.join(output_dir, existing_file)):
...
def main():
    parser = argparse.ArgumentParser(description="Google Photos Shared Album Downloader")
    parser.add_argument("url", help="Shared album URL")
    parser.add_argument("-o", "--output-dir", default=".", help="Output directory")
    parser.add_argument("-s", "--sync", action="store_true", help="Sync mode (skip existing)")
    parser.add_argument("-v", "--verbose", action="store_true", help="Verbose output")
    args = parser.parse_args()

    # 1. Resolve URL
    full_url = resolve_url(args.url, args.verbose)
    if not full_url: sys.exit(1)
        ...
        for script in script_tags:
            ...
            if script_content and 'AF_initDataCallback' in script_content:
                ...
                if match:
                    ...
                    cleaned_json_str = re.sub(r'([{,])\s*([a-zA-Z_][a-zA-Z0-9_]*)\s*:', r'\1"\2":', cleaned_json_str)

                    try:
                        parsed_callback_data = json.loads(cleaned_json_str)
                        data_array = parsed_callback_data.get('data')

                        if data_array and isinstance(data_array, list) and len(data_array) > 1 and isinstance(data_array[1], list):
                            photos_data = data_array[1]
                            for photo_entry in photos_data:
                                if isinstance(photo_entry, list) and len(photo_entry) > 1 and \
                                   isinstance(photo_entry[1], list) and len(photo_entry[1]) > 0 and \
                                   isinstance(photo_entry[1][0], str) and "googleusercontent.com" in photo_entry[1][0]:
    ...
    console.print(f"Local: [bold]{skipped_count}[/bold] | To Download: [bold]{len(to_download)}[/bold]")
